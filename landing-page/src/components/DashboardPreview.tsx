    ...
    <section className="py-24 px-4 overflow-hidden relative">
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Full visibility into your night shift</h2>
          <p className="text-gray-400 max-w-2xl mx-auto">Web dashboard included. Real-time updates via SSE. No extra hosting required.</p>
        </div>
            ...
            <div className="rounded-xl border border-gray-800/80 bg-[#0a0a0a]/90 overflow-hidden flex flex-col h-[500px]">
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                <div className="hidden md:flex w-48 border-r border-gray-800/50 p-4 flex-col gap-2 bg-gray-900/20">
                  ...
                  <div className="h-8 w-full bg-indigo-500/10 border border-indigo-500/20 rounded text-indigo-400 text-xs flex items-center px-2">Board</div>
                  <div className="h-8 w-full bg-transparent hover:bg-gray-800/50 rounded text-gray-400 text-xs flex items-center px-2 transition-colors">Pull Requests</div>
                  <div className="h-8 w-full bg-transparent hover:bg-gray-800/50 rounded text-gray-400 text-xs flex items-center px-2 transition-colors">Agents</div>
                  <div className="h-8 w-full bg-transparent hover:bg-gray-800/50 rounded text-gray-400 text-xs flex items-center px-2 transition-colors">Settings</div>
                </div>
                ...
                <div className="flex-1 p-4 md:p-6 flex flex-col gap-6 overflow-y-auto bg-gradient-to-br from-transparent to-gray-900/20">
                  ...
                  <div className="flex flex-wrap md:flex-nowrap gap-4">
                    {['Executor', 'Reviewer', 'QA', 'Auditor', 'Slicer'].map((agent, i) => (
                      <div key={i} className="flex-1 min-w-[100px] bg-gray-800/40 border border-white/5 rounded-lg p-3 backdrop-blur-sm">
                        <div className="text-xs text-gray-400 mb-1">{agent}</div>
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${i === 0 ? 'bg-amber-500 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.8)]' : 'bg-gray-600'}`}></div>
                          <div className="text-xs font-mono text-gray-300">{i === 0 ? 'Running...' : 'Idle'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex-1 flex overflow-x-auto md:overflow-visible gap-4 pb-4 md:pb-0 snap-x snap-mandatory hide-scrollbar">
                    {['Ready', 'In Progress', 'Review', 'Done'].map((col, i) => (
                      <div key={i} className="flex-none w-[260px] md:w-auto md:flex-1 bg-gray-800/20 border border-white/5 rounded-lg p-3 flex flex-col gap-3 min-h-[200px] snap-start">
                        <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{col}</div>
                        {i === 0 && (
                          <>
                            <div className="bg-gray-800/80 border border-gray-700/50 rounded p-3 shadow-sm hover:border-gray-600 transition-colors">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-[10px] bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded">P2</span>
                              </div>
                            </div>
                          </>
                        )}
                        {i === 1 && (
                          <div className="bg-gray-800/90 border border-indigo-500/50 rounded p-3 shadow-[0_0_15px_-3px_rgba(99,102,241,0.2)] relative overflow-hidden">
                            ...
                            <div className="text-xs text-gray-200 mb-2 mt-1">Implement user settings</div>
                            <div className="flex justify-between items-center">
                        ...
                        {i === 2 && (
                          <div className="bg-gray-800/80 border border-gray-700/50 rounded p-3 shadow-sm hover:border-gray-600 transition-colors">
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded">Score: 92</span>
                            </div>
                          </div>
                        )}
                      </div>
