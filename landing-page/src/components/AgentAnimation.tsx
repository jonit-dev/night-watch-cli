...
import { CheckCircle2, CheckSquare, Code2, GitPullRequest, Terminal } from 'lucide-react';
...
const Ticket = ({ frame }: { frame: number }) => {
  const x = interpolate(frame, [50, 80, 140, 170, 230, 260], [30, 230, 230, 430, 430, 630], {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 14, color: '#f3f4f6', fontWeight: 600, marginBottom: 8 }}>
        Auth Flow
      </div>
...
const ExecutorAgent = ({ frame }: { frame: number }) => {
  const opacity = interpolate(frame, [20, 30, 170, 180], [0, 1, 1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const x = interpolate(frame, [50, 80, 140, 170], [30, 230, 230, 430], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const y = interpolate(frame, [20, 30], [150, 130], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
...
const ReviewerAgent = ({ frame }: { frame: number }) => {
  const opacity = interpolate(frame, [170, 180, 270, 280], [0, 1, 1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const x = interpolate(frame, [230, 260], [430, 630], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const y = interpolate(frame, [170, 180], [150, 130], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
...
const PRBadge = ({ frame }: { frame: number }) => {
  ...
  const opacity = interpolate(frame, [250, 260], [1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
