...
export const JOB_DEFINITIONS: IJobDefinition[] = [
  ...
  { id: 'optimizer', label: 'Optimizer', processName: 'optimizer', color: { bg: 'bg-emerald-500', border: 'border-emerald-500/60' } },
...
export const WEB_JOB_REGISTRY: IWebJobDefinition[] = [
  ...
  },
  {
    ...
    color: { bg: 'bg-emerald-500', border: 'border-emerald-500/60' },
    triggerEndpoint: '/api/actions/optimize',
    scheduleTemplateKey: 'optimizer',
    getEnabled: (config) => config.optimizer?.enabled ?? false,
    getSchedule: (config) => config.optimizer?.schedule ?? '20 4 * * 2',
    settingsSection: 'advanced',
    buildEnabledPatch: (enabled, config) =>
      ({ optimizer: { ...(config.optimizer ?? {}), enabled } as IOptimizerConfig }),
  },
  {
    id: 'ux',
    label: 'UX',
