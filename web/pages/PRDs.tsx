...
import { LayoutList, LayoutGrid, Play, AlertCircle, RotateCcw, ArrowUp, ArrowDown, ArrowUpDown, Loader2 } from 'lucide-react';
...
const PRDs: React.FC = () => {
  ...
  const handleRetry = async (prdName: string) => {
    setIsRetrying(true);
    try {
            ...
            <table className="min-w-full divide-y divide-slate-800">
              <thead className="bg-slate-950/50">
                <tr>
                  ...
                  <th scope="col" className="relative px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
                ...
                {sortedPrds.map((prd) => (
                  <tr key={prd.name} className="hover:bg-slate-800/50 group cursor-pointer transition-colors" onClick={() => setSelectedPRD(prd)}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      ...
                      {(prd.status === 'blocked' || prd.status === 'pending-review') && (
                ...
                {sortedPrds.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-500">
                      {prds.length === 0 ? 'No PRDs found.' : `No PRDs match the "${filter}" filter.`}
                    </td>
                  </tr>
                )}
              </tbody>
           ...
           <div className="relative w-full max-w-xl bg-slate-900 h-full shadow-2xl flex flex-col transform transition-transform animate-in slide-in-from-right duration-300 border-l border-slate-800">
              ...
              <div className="p-4 border-t border-slate-800 bg-slate-900 flex space-x-3">
                 {selectedPRD.status === 'done' ? (
                   <Button className="flex-1" onClick={() => handleRetry(selectedPRD.name)} disabled={isRetrying}>
