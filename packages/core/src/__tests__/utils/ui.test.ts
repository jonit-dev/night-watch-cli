...
describe('UI utilities', () => {
  ...
  describe('step()', () => {
    it('should format step as a wizard progress row', () => {
      step(1, 9, 'Checking git repository...');
      const progressOutput = consoleSpy.mock.calls[0][0];
      const messageOutput = consoleSpy.mock.calls[1][0];
      expect(progressOutput).toContain('Step 01/09');
      expect(progressOutput).toContain('11%');
      expect(messageOutput).toBe('  Checking git repository...');
    });
  });
});
