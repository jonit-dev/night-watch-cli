...
describe('status-data utilities', () => {
  ...
  describe('executorLockPath / reviewerLockPath / qaLockPath / auditLockPath', () => {
  ...
  describe('collectLogInfo', () => {
    it('should collect info for executor/reviewer/qa/audit logs', () => {
    ...
    it('should use correct file names (executor.log, reviewer.log, night-watch-qa.log, audit.log)', () => {
      const logDir = path.join(tempDir, 'logs');
      fs.mkdirSync(logDir, { recursive: true });
      fs.writeFileSync(path.join(logDir, 'executor.log'), 'Executor line 1');
  ...
  describe('fetchStatusSnapshot', () => {
    it('should return all expected fields', () => {
      ...
      expect(snapshot.processes).toHaveLength(4);
      ...
      expect(snapshot.logs).toHaveLength(4);
