...
import type { ExecutionOutcome } from '@night-watch/core';

const VALID_OUTCOMES: ExecutionOutcome[] = ['success', 'failure', 'timeout', 'rate_limited'];
...
export function historyCommand(program: Command): void {
  ...
  history
    ...
    .action(
      (
        ...
        recordExecution(projectDir, prdFile, outcome as ExecutionOutcome, exitCode, attempt);
      },
    );
  ...
  history
    ...
    .action((projectDir: string, prdFile: string, options: { cooldown: string }) => {
