...
import { GLOBAL_CONFIG_DIR } from "@night-watch/core/constants.js";
import { migrateJsonToSqlite } from "@night-watch/core/storage/json-state-migrator.js";
  ...
  state
    ...
    .action((opts: { dryRun?: boolean }) => {
      const nightWatchHome =
        process.env.NIGHT_WATCH_HOME ||
        path.join(os.homedir(), GLOBAL_CONFIG_DIR);

      if (opts.dryRun) {
        console.log(
          chalk.cyan("Dry-run mode: no changes will be made.\n")
        );
        console.log(
          `Legacy JSON files that would be migrated from: ${chalk.bold(nightWatchHome)}`
        );
      ...
      if (result.alreadyMigrated) {
        console.log(
          chalk.yellow("Migration already completed previously. Nothing to do.")
        );
        return;
      }
      ...
      console.log(
        `  Projects migrated:         ${chalk.cyan(String(result.projectsMigrated))}`
      );
      console.log(
        `  History records migrated:  ${chalk.cyan(String(result.historyRecordsMigrated))}`
      );
      console.log(
        `  PRD states migrated:       ${chalk.cyan(String(result.prdStatesMigrated))}`
      );
      console.log(
