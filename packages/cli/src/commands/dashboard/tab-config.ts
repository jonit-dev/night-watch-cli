...
export function createConfigTab(): ITab {
  ...
  function buildListItems(config: INightWatchConfig): string[] {
    return CONFIG_FIELDS.map((field) => {
      ...
      const editHint =
        field.type === 'keyvalue' || field.type === 'webhooks'
  ...
  function showKeyValueEditor(ctx: ITabContext, config: INightWatchConfig) {
    ...
    function refreshKvList() {
      kvList.setItems(buildItems() as unknown as string[]);
      (kvList as unknown as { height: number }).height = Math.min(
        Object.keys(editableEnv).length + 4,
        20,
      );
      ctx.screen.render();
    }
  ...
  function showWebhookEditor(ctx: ITabContext, config: INightWatchConfig) {
    ...
    const editableWebhooks: IWebhookConfig[] = currentNotif.webhooks.map((w) => ({
      ...w,
      events: [...w.events],
    }));
    ...
    function selectEvents(
      ...
      cb: (events: NotificationEvent[] | null) => void,
    ) {
  ...
  function showEditor(ctx: ITabContext, field: IConfigField, config: INightWatchConfig) {
    if (field.type === 'keyvalue') {
      ...
      [
        ...
        () => {
          ...
          const scheduleChanged =
            'cronSchedule' in pendingChanges ||
          ...
          if (scheduleChanged) {
            ...
            if (!installResult.success) {
              ctx.showMessage(
                `Config saved but cron reinstall failed: ${installResult.error}`,
                'error',
              );
            } else {
