...
describe('is_claimed', () => {
  it('returns 0 for active claim', () => {
    ...
    const result = sourceHelpersAndRun(
      `is_claimed "${prdDir}" "01-test-prd.md" 7200 && echo "claimed"`,
    );
  ...
  it('returns 1 for stale claim', () => {
    ...
    const claimPath = path.join(prdDir, '01-test-prd.md.claim');
    fs.writeFileSync(claimPath, '{"timestamp":1000000000,"hostname":"test","pid":1}\n');
...
describe('build_provider_prompt', () => {
  it('prefixes the prompt with /goal by default', () => {
    const result = sourceHelpersAndRun(`build_provider_prompt "PRDs/01-test.md" "Implement it"`);

    expect(result.status).toBe(0);
    expect(result.stdout).toBe('/goal PRDs/01-test.md\n\nImplement it');
  });

  it('returns the raw prompt when /goal wrapping is disabled', () => {
    const result = sourceHelpersAndRun(`build_provider_prompt "PRDs/01-test.md" "Implement it"`, {
      NW_PROVIDER_USE_GOAL_COMMAND: '0',
    });

    expect(result.status).toBe(0);
    expect(result.stdout).toBe('Implement it');
  });
});
