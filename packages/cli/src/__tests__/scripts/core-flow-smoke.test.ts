...
describe('core flow smoke tests (bash scripts)', () => {
  ...
  it('reviewer should return non-zero when provider fails', () => {
    ...
    fs.writeFileSync(
      ...
      '#!/usr/bin/env bash\n' +
        'if [[ "$1" == "pr" && "$2" == "list" ]]; then\n' +
        '  if [[ "$*" == *"number,headRefName"* ]]; then\n' +
        "    echo $'1\\tfeat/reviewer-failure'\n" +
        '  else\n' +
        "    echo 'feat/reviewer-failure'\n" +
        '  fi\n' +
        '  exit 0\n' +
        'fi\n' +
        ...
        '  exit 0\n' +
        'fi\n' +
        'if [[ "$1" == "pr" && "$2" == "view" && "$*" == *"mergeStateStatus"* ]]; then\n' +
        "  echo 'DIRTY'\n" +
        '  exit 0\n' +
        'fi\n' +
        'exit 0\n',
  ...
  it('executor should not skip PRD when open branch only contains PRD name as substring', () => {
    const projectDir = mkTempDir('nw-smoke-executor-substring-');
    initGitRepo(projectDir);
    createPrd(projectDir, 'api');
    commitAll(projectDir, 'add api PRD');

    const fakeBin = mkTempDir('nw-smoke-executor-substring-bin-');
    const invokedFlag = path.join(projectDir, '.provider-invoked');

    fs.writeFileSync(
      path.join(fakeBin, 'claude'),
      '#!/usr/bin/env bash\n' + 'touch "$NW_SMOKE_PROVIDER_FLAG"\n' + 'exit 0\n',
      { encoding: 'utf-8', mode: 0o755 },
    );
    ...
    fs.writeFileSync(
      ...
      '#!/usr/bin/env bash\n' +
        ...
        '  done\n' +
        '  if [[ "$state" == "open" ]]; then\n' +
        "    echo 'night-watch/graph-api-hardening'\n" +
        '  fi\n' +
        '  exit 0\n' +
        'fi\n' +
        'exit 0\n',
    ...
    const result = runScript(executorScript, projectDir, {
      ...
      NW_PRD_DIR: 'docs/PRDs/night-watch',
      NW_DEFAULT_BRANCH: 'main',
      NW_SMOKE_PROVIDER_FLAG: invokedFlag,
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toContain('NIGHT_WATCH_RESULT:failure_no_pr_after_success');
    expect(result.stdout).not.toContain('NIGHT_WATCH_RESULT:skip_no_eligible_prd');
    expect(fs.existsSync(invokedFlag)).toBe(true);
  });
