...
describe('board commands', () => {
  ...
  describe('board create-prd', () => {
    ...
    it('passes body text when --body is provided', async () => {
      ...
      await runCommand(['board', 'create-prd', 'PRD with body', '--body', 'The description']);
    ...
    it('passes label when --label is provided', async () => {
      ...
      await runCommand(['board', 'create-prd', 'Labelled PRD', '--label', 'prd']);
  ...
  describe('board next-issue', () => {
    ...
    it('returns all issues as JSON array with --all --json', async () => {
      const issues = [
        makeIssue({ number: 1, title: 'First', column: 'Ready', labels: ['P0'] }),
        makeIssue({ number: 2, title: 'Second', column: 'Ready', labels: ['P1'] }),
        makeIssue({ number: 3, title: 'Third', column: 'Ready' }),
      ];
      mockProvider.getIssuesByColumn.mockResolvedValue(issues);

      await runCommand(['board', 'next-issue', '--all', '--json']);

      const output = consoleSpy.mock.calls[0][0];
      const parsed = JSON.parse(output);
      expect(parsed).toHaveLength(3);
      // P0 first, then P1, then unlabeled
      expect(parsed[0].number).toBe(1);
      expect(parsed[1].number).toBe(2);
      expect(parsed[2].number).toBe(3);
    });

    it('outputs empty array for --all --json when column is empty', async () => {
      mockProvider.getIssuesByColumn.mockResolvedValue([]);
  ...
  describe('board comment', () => {
    it('adds a comment to an issue', async () => {
      ...
      await runCommand(['board', 'comment', '42', '--body', 'Great progress!']);
