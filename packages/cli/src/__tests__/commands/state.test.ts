...
function writeFixtures(): void {
  ...
  const history = {
    '/home/user/my-project': {
      ...
      'docs/PRDs/other.md': {
        records: [{ timestamp: 1700200000000, outcome: 'timeout', exitCode: 124, attempt: 1 }],
      },
    },
    '/home/user/other-project': {
      'docs/PRDs/thing.md': {
        records: [{ timestamp: 1700300000000, outcome: 'rate_limited', exitCode: 2, attempt: 3 }],
      },
    },
  };
  fs.writeFileSync(path.join(tmpDir, 'history.json'), JSON.stringify(history, null, 2));
  ...
  fs.writeFileSync(path.join(tmpDir, 'prd-states.json'), JSON.stringify(prdStates, null, 2));
}
...
describe('night-watch state migrate', () => {
  ...
  it('re-running migration is safe (idempotent)', () => {
    ...
    const { exitCode: secondExitCode, stdout: secondStdout } = runCli('state migrate');
  ...
  it('creates backup files for each legacy JSON that existed', () => {
    ...
    const backupProjects = JSON.parse(
      fs.readFileSync(path.join(migrationBackupDir, 'projects.json'), 'utf-8'),
    ) as unknown[];
