...
function getMainHelp(): string {
  if (!cachedHelpOutput) {
    cachedHelpOutput = execSync(`node "${CLI_PATH}" --help`, {
      encoding: 'utf-8',
      timeout: 10000,
    });
  }
...
function getDashboardHelp(): string {
  if (!cachedDashboardHelpOutput) {
    cachedDashboardHelpOutput = execSync(`node "${CLI_PATH}" dashboard --help`, {
      encoding: 'utf-8',
      timeout: 10000,
    });
  }
...
describe('dashboard command', () => {
      ...
      const prds = [
        {
          ...
          unmetDependencies: ['phase1', 'phase2'],
        },
      ];
  ...
  describe('renderLogPane', () => {
    ...
    it('should render empty message when no log files exist', () => {
      const logs = [
        {
          name: 'executor',
          path: '/tmp/nonexistent/executor.log',
          exists: false,
          size: 0,
          lastLines: [],
        },
        {
          name: 'reviewer',
          path: '/tmp/nonexistent/reviewer.log',
          exists: false,
          size: 0,
          lastLines: [],
        },
  ...
  describe('render functions with empty data', () => {
    ...
    it('should handle single-item arrays without crash', () => {
      ...
      const processResult = renderProcessPane([{ name: 'executor', running: false, pid: null }]);
