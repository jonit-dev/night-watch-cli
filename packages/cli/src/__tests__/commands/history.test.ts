...
describe('night-watch history', () => {
  ...
  it('should exit 1 when PRD is eligible (no history)', () => {
    const { exitCode } = runCli(`history check /tmp/test-project none.md --cooldown 7200`);
    expect(exitCode).toBe(1);
  });

  it('should exit 1 when last record is success', () => {
    runCli(`history record /tmp/test-project test.md success --exit-code 0`);
    const { exitCode } = runCli(`history check /tmp/test-project test.md --cooldown 7200`);
    expect(exitCode).toBe(1);
  });

  it('should reject invalid outcome', () => {
    const { exitCode } = runCli(`history record /tmp/test-project test.md invalid_outcome`);
