 ...
 *
 * Phase 5: Service classes (NotificationService, StatusService, RoadmapService) and
 * Slack classes (SlackClient, DeliberationEngine, SlackInteractionListener) are also
 * registered. Slack instances are registered via registerInstance since they require
 * runtime configuration values (tokens, config) that are not available at container
 * bootstrap time — they are registered by the composition root (server/index.ts) after
...
export function initContainer(projectDir: string): void {
  ...
  // Run migrations so the schema is ready before repositories are resolved
  runMigrations(db);
  ...
  // Register server-layer service singletons. These have no constructor dependencies
  // on the database — they delegate to utility functions — so they can be registered
  // as plain singletons without any @inject tokens.
  container.registerSingleton(NotificationService);
  container.registerSingleton(StatusService);
  container.registerSingleton(RoadmapService);
}
