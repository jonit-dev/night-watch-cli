...
import type { IAgentPersona, IDiscussionTrigger, ISlackDiscussion } from '../../../shared/types.js';
...
vi.mock('../../storage/repositories/index.js', () => ({
...
describe('DeliberationEngine board routing', () => {
  ...
  it('does not fall back to listener config board when target project board is missing', async () => {
    ...
    const postedMessages = slackClient.postAsAgent.mock.calls.map((call) => String(call[1]));
    expect(postedMessages.some((msg) => msg.includes('No board configured'))).toBe(true);
  });
});
