...
- Dev: `{ provider: 'anthropic', model: 'claude-haiku-4-5' }` — fast, cheap implementer updates
...
```
This means Carlos can deliberate using `claude-opus-4-6` while Priya uses `claude-haiku-4-5`, in the same discussion thread. Custom env vars in `modelConfig.envVars` are injected as process-level overrides scoped to that single API call, then restored.
